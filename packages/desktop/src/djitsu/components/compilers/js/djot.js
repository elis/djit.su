...
export default async function djotCompiler(source, options = {}) {
  ...
  const results = context.results?.length
    ? lines.lines.map(([line, ...rest], lineIndex) => [
        {
          ...line,
          result: lines.resultMap.has(lineIndex)
            ? context.results[lines.resultMap.get(lineIndex)]
            : null
        },
        ...rest
      ])
  ...
  for (let nodeIndex = 0; nodeIndex < ast.body.length; ++nodeIndex) {
    ...
    if (prevNLs) {
      ...
      for (let i = 0; i < prevNLs - (hasLine ? 1 : 0); ++i) {
        lines.push([
          { newline: true, line: i, total: prevNLs - (hasLine ? 1 : 0) }
        ])
      }
    }
