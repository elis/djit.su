...
import { hooks as dhooks } from '@djitsu/utils'
...
import { Tool, toolMaker, useTool } from 'editorjs-react-tool'

const { useStates } = dhooks
  ...
  useEffect(() => {
    if (errors?.length > 0) {
      ...
      if (haveName) {
        ...
        if (myOffence)
          setInput('code', (v) => ({ ...v, error: myOffence.error }))
      }
