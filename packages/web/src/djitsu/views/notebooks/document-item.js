...
const DocumentItem = ({ item, onUpdate }) => {
  // const doks = useDocuments()
  // const doc = useMemo(
  //   () =>
  //     // console.log('🗄 doks.state', doks.state) ||
  //     doks.state?.documents?.[item.docid],
  //   [doks.state?.documents]
  // )
  const [loads, setLoading] = useStates({
    delete: false
  })
  const [showDetails, setShowDetails] = useState()
  // item?.docid === 'q9YC6egdBkU22'
  ...
  // useEffect(() => {
  //   if (showDetails) {
  //     // console.log('📑 What is item, doc:', item, doc)
  //   }
  // }, [item, doc, showDetails])
      ...
      <div className='data'>
        <Space direction='horizontal' size='small' align='end'>
          <Statistic
            ...
            // value={selfLoaded ? doc?.stats?.likes : '...'}
          ...
          <Statistic
            ...
            // value={selfLoaded ? doc?.stats?.stars : '...'}
          ...
          <Statistic
            ...
            // value={selfLoaded ? doc?.stats?.forks : '...'}
        ...
        {/* {doc?.pubver ? (
          ...
          <Statistic title='Shared Document' value={`${item.docid}`} />
        ) : (
          <Statistic
            title='Private Document'
            value={doc?.username ? `@${doc?.username}` : '...'}
          />
        )}
        {doc?.size > 0 ? (
          <Statistic
            title='Document Size'
            suffix={formatBytes(doc?.size)[1].toLowerCase()}
            value={selfLoaded ? doc?.size : '...'}
            formatter={value => (value > 0 ? formatBytes(value)[0] : '')}
          />
        ) : (
          <Statistic title='Document Size' value={selfLoaded ? 0 : '...'} />
        )}
        <Statistic
          title='Blocks Count'
          value={selfLoaded ? doc?.blocksCount : '...'}
        /> */}
      </div>
    ),
    [
...
export const documentItem = (onUpdate) => (item) => {
